...
st.markdown("""
### Objectives & Contrains:
1.  **Minimize Cost**: Malaysian ToU Tariff (Peak: RM 0.50 [08:00-22:00], Off-Peak: RM 0.30).
2.  **Minimize Discomfort**: Reducing shift from Preferred start times.
...
def load_and_clean_data():
    # Loading the user-provided project_benchmark_data.csv
...
# Split dataset into fixed and shiftable
...
def fitness(individual):
    ...
    Includes Electricity Cost, Discomfort penalty, and Power Limit penalties.
    """
    total_cost = 0
    total_discomfort = 0
    penalty = 0
    ...
    # 3. Peak Power Constraint Penalty (Rule: Max 5.0 kW)
...
def run_ga():
    ...
    for gen in range(generations):
        ...
        while len(new_population) < pop_size:
            p1 = min(random.sample(population, 3), key=fitness)
            p2 = min(random.sample(population, 3), key=fitness)
...
if st.button("🚀 Run Scheduling Optimization"):
    ...
    daily_baseline = get_raw_cost(baseline_power)
    daily_optimized = get_raw_cost(final_power)

    # --- Section: Metrics ---
    st.subheader("Optimization Summary")
    m1, m2, m3 = st.columns(3)
    m1.metric("Daily Baseline Cost", f"RM {daily_baseline:.2f}")
    m2.metric("Daily Optimized Cost", f"RM {daily_optimized:.2f}")
    m3.metric("Daily Savings", f"RM {daily_baseline - daily_optimized:.2f}")
    ...
    with col_a:
        ...
        ax.axhline(y=MAX_POWER_LIMIT, color='red', linestyle='--', label='5.0 kW Limit')
    ...
    for _, row in fixed_apps.iterrows():
        final_table.append({
            ...
            "Type": "Non-Shiftable",
            "Preferred Start": f"{row['Preferred']}:00",
            "Scheduled Start": f"{row['Preferred']}:00",
            "Duration (Hours)": row['Duration'],
            "End Time": f"{(row['Preferred'] + row['Duration']) % 24}:00",
            "Power (kW)": row['Power_kW']
        })
    ...
    if max(final_power) > MAX_POWER_LIMIT:
        st.error(f"⚠️ Peak Power reached {max(final_power):.2f} kW. Limit of {MAX_POWER_LIMIT} kW exceeded!")
    else:
        st.success(f"✅ Peak Power successfully maintained below {MAX_POWER_LIMIT} kW.")
